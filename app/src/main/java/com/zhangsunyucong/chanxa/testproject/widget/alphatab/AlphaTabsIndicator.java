package com.zhangsunyucong.chanxa.testproject.widget.alphatab;
    ...
    private void init() {
        ...
        for (int i = 0; i < mChildCounts; i++) {
            if(getChildAt(i) instanceof ViewGroup) {
                ...
                if (viewGroup.getChildAt(0) instanceof AlphaTabView) {
                    AlphaTabView tabView = (AlphaTabView) viewGroup.getChildAt(0);
                    mTabViews.add(tabView);
                    //设置点击监听
                    tabView.setOnClickListener(new MyOnClickListener(i));
                } else {
                    throw new IllegalArgumentException("直接子View必须是TabView");
                }
